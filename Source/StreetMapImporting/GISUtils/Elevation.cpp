...
private:
	...
	void DownloadFile()
	{
		TMap<FString, FStringFormatArg> UrlArgs;
		UrlArgs.Add(TEXT("tilesize"),FStringFormatArg(TiledMap.TileWidth));
		UrlArgs.Add(TEXT("z"),FStringFormatArg(Z));
		UrlArgs.Add(TEXT("x"), FStringFormatArg(X));
		UrlArgs.Add(TEXT("y"), FStringFormatArg(Y));
		FString URL = FString::Format(*TiledMap.URLTemplate, UrlArgs);
